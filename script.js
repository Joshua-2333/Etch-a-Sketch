...
function convertRGBAToHexA(rgba, forceRemoveAlpha = false) {
    return "#" + rgba.replace(/^rgba?\(|\s+|\)$/g, '') // Get's rgba / rgb string values
      ...
      .join("") // Puts the array to together to create a string
}
...
slider.oninput = function() {
    ...
    sliderValue.textContent = `${this.value} x ${this.value} (Number of Squares)`;
    removeGridSquares();
    createGridSquares();
}
sliderValue.textContent = `${slider.value} x ${slider.value} (Number of Squares)`;
